...
public class Localizer {
	...
	public static float ROTATION_SPEED = 35;
	...
	private final double TILE_DIMENSION = 30;
	...
	public void doLocalization() {
		...
		if (locType == LocalizationType.FALLING_EDGE) {
			...
			while ((usDistance = getFilteredData()) < USDISTANCE_THRESHOLD) { // (we don't want the same wall
																				// to be detected)
				LCD.drawString("US: " + usDistance + "  ", 0, 0);
			}
			...
			Sound.beep();
			odo.correctTheta(deltaTheta); // ensuring theta doesnt got out of bounds
			LCD.drawString("Theta: " + odo.getAng() + "  ", 0, 4);

			// now we MEASURE
			// measuring x and y can be found below
			if (isMeasured) { // x, y already measured with respect to new (0,0)!
				LCD.drawString("Finding 0  ", 0, 5);
				navigate.setRotationSpeed(10); // rotate slowly
				for (;;) // orienting robot
				{ // EXIT ONLY IF ROBOT FACING NORTH (give or take)
			...
			/*
			 * The robot should turn until it sees the wall, then look for the "rising edges:" the points
			 * where it no longer sees the wall. This is very similar to the FALLING_EDGE routine, but the
			 * robot will face toward the wall for most of it.
			 */
			usDistance = getFilteredData(); // measure USdistance
			...
			while ((usDistance = getFilteredData()) == USDISTANCE_THRESHOLD) { // wait to clear off first
																				// wall!
				LCD.drawString("US: " + usDistance + "  ", 0, 0);
			}
			// 2nd wall cleared !!
		...
		if (!isMeasured) {
			...
			for (;;) // Start with x
			{
				...
				if (error < 0.5) // ERROR SHOULD BE <0.5
					break;
				navigate.setRotationSpeed((float) (-error - 20)); // Robot going clockwise; accelerated by
																	// random constant
				try {
					Thread.sleep(50);
				} catch (InterruptedException e) {
				} // rotate until error reaches threshold
			}
			navigate.stopMoving();
			...
			navigate.stopMoving();
			double yPrime = usDistance = getFilteredData();
			...
			odo.setX(-TILE_DIMENSION + xCoord); // now we move (0,0) from arena corner to intersection of 2
												// gridlines !!
			odo.setY(-TILE_DIMENSION + yCoord);
	...
	private float getFilteredData() {
		float distance = usSensor.scan();

		if (distance > 35) // filter out large values
			distance = 35; // thus makes anything above the threshold distance irrelevant
